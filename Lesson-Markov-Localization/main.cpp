...
std::vector<float> initialize_priors(int map_size,
                                     std::vector<float> landmark_positions,
                                     float control_stdev);

float motion_model(float pseudo_position, float movement,
                   std::vector<float> priors, int map_size, int control_stdev);
...
int main() {
  ...
  //define observations vector, each inner vector represents a set of observation_stdev
  //for a time step
  std::vector<std::vector<float>> sensor_obs {{1,7,12,21}, {0,6,11,20}, {5,10,19},
                                              {4,9,18}, {3,8,17}, {2,7,16}, {1,6,15},
                                              {0,5,14}, {4,13}, {3,12}, {2,11},
                                              {1,10}, {0,9}, {8}, {7}, {6}, {5},
                                              {4}, {3}, {2}, {1}, {0}, {}, {}, {}};
...
std::vector<float> pseudo_range_estimator(std::vector<float> landmark_positions,
  ...
  //loop over number of landmarks and estimates pseudo ranges
...
float motion_model(float pseudo_position, float movement,
  ...
  //step over state space for all possible positions x (convolutions)
